...
# def findGreater(hand1, hand2, pos):#assumes they are in the same tier    
#     if((hand1 & (0<<pos)) == hand2 &(0<<pos)):
#         return findGreater(hand1, hand2, pos-1)
#     elif((hand1 & (0<<pos)) > hand2 &(0<<pos)):
#         return True
#     else: return False
...
for i, line in enumerate(f):#read the input file
    ...
    #convert face cards to hex numbers
    for j, card in enumerate(game.hand):
        if card == 'T':
            game.hand[j] = 'A'
        elif card == 'J':
            game.hand[j] = '1'#swap this to 'B' to run part 1
        elif card == 'Q':
            game.hand[j] = 'C'
        elif card == 'K':
             game.hand[j] = 'D'
...
for i, tierGroup in enumerate(games):
    ...
    for j, round in enumerate(tierGroup):
        ...
        print(games[i][j].hexHand, "".join(games[i][j].originalHand), currentRank, games[i][j].tier)
   

print("Part 1 Answer: ", answer1)
#don't feel like making this work for part 1 and part 2
